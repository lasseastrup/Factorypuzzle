...
- **Junctions are fittings, not machines.** A splitter used to be a hexagonal drum nearly as tall as a smelter, so a row of them read as a row of buildings. It is now a low plate at about belt height, roughly a quarter of a machine's height, with no plinth — it belongs to the belt run rather than standing on its own foundation.
