...
- **A belt works out which way round it goes.** The drawn direction wins whenever it is legal, because that is what the player asked for. When it is not — dragging from a junction to a miner, or out of an output already in use — the reverse is tried before refusing. In those cases only one direction was ever possible, so making the player discover which one is busywork. Where both directions are legal, the drawn one is respected and never second-guessed.
- **One Junction does both jobs, and you make it by branching a belt.** There is no splitter and no merger: a junction sums whatever arrives and divides it evenly among whatever leaves, which is the same arithmetic in both directions, so the distinction only ever existed in the player's head. Up to three belts each way, and which way a given belt runs is settled when it is drawn.
  ...
  **A junction sits on the belt, and splicing never reshapes it.** The two halves of the original run are exact sub-paths of it, split at the junction, not re-routed. Re-routing them replaced a run the player had shaped by hand with two straight lines.

  Two mistakes here are worth recording because they had the same origin — treating the belt being spliced as if it were any other belt:

  - `canPlace` counts belts as obstacles, so the belt being spliced blocked its own junction: every candidate position is by definition a point on that belt. Splices were refused as "no room" with the plot empty around them. It now takes a link to discount.
  - The fallback search for a clear position then moved the junction one to three metres *off* the belt, which is what made the run change shape on the splices that did succeed. The search now walks *along* the belt and never away from it.
...
- **A junction has no model.** Nothing is drawn for it: the belts show it themselves by meeting at a point and forking, which is what a fork in a conveyor looks like. Every belt at a junction terminates on the point rather than on a footprint edge, so the ribbons form one continuous division, and no drum is drawn there — a drum belongs where a run *terminates*, and at a fork the belts continue. Three drums meeting would draw three belt ends instead of one line dividing.
  ...
  A junction can still be selected by tapping the fork, since the fork is where it visibly is. Removing one that merely passes material through **rejoins the line** rather than severing it — the player never placed a fitting, they branched a belt, so deleting the branch should put the belt back.

- **Junctions were briefly fittings, not machines.** A splitter used to be a hexagonal drum nearly as tall as a smelter, so a row of them read as a row of buildings. It is now a low plate at about belt height, roughly a quarter of a machine's height, with no plinth — it belongs to the belt run rather than standing on its own foundation.
- **Belt tiers carry more AND move faster:** Mk1 60/min at 1.4 m/s, Mk2 120/min at 2.1, Mk3 240/min at 2.9. Capacity alone made the tiers indistinguishable on the plot — the only visible difference was how closely the items sat, which at low flow is no difference at all. Each tier gets its own surface mesh so its cleats scroll at the speed of the belt carrying them.
- **Pointing at a belt means the nearest belt, at the belt's own height.** Hit-testing returned the first belt within tolerance, and iteration order is creation order, so aiming at one belt could connect to an older one running alongside. Worse, the test used the ground point under the cursor: a belt stands 0.34 m up, which at this camera pitch puts that point about 0.25 m beyond it — three quarters of the whole tolerance, biased consistently toward whatever lies behind. Both had to be wrong together for the bug to be as confusing as it was.
- **Machines have one output *connection*, not one output *location*.** A belt meets a machine at the point on its perimeter nearest whatever it is connecting to. The earlier design fixed each port to a single spot on one face, which conflated two different things: the logical constraint the puzzle needs (a machine feeds one belt, so a splitter is required to branch) and a geometric one it does not (that belt must leave from *here*). The geometric half only created work — a machine turned toward the plot edge forced its belt to loop out of bounds. Dropping it changed no level design, because every level depends on the count and none on the position.
- **Factories keep designed port positions**, because there a port's place on the box is the only way to tell one terminal from another.
- **Placement snaps to a metre grid** — the footprint *edges*, not the centre, so a 2 m machine and a 3 m one still line up. The grid is invisible: it exists to make neat rows effortless, not to be obeyed.
- **Facing machines meet halfway.** When two attachment faces look at each other, the crossing run is placed midway between them rather than a fixed distance from each. Using a fixed distance leaves the two approach runs on lines a few centimetres apart, and that gap becomes an elbow too short to fillet. This was the single most persistent source of sharp corners in the router, and it took four separate attempts to find — each earlier fix removed one symptom and left the cause.
...
**Coplanar faces are the recurring bug in this codebase.** Three separate belt parts have now had their top face sitting exactly on the ribbon plane, each fighting it and surfacing as dark marks through the belt: first the support posts, then the cross-beam that replaced them, then — found by measuring rather than being reported — the end drums, which were positioned from the item ride height instead of the belt surface and floated 0.19 m clear. Every one of these is geometry, so every one can be asserted. `test/harness.js` now measures the transformed bounds of every belt part and requires each structural piece to sit strictly below the ribbon, with the side rails as the single deliberate exception since they stand proud to keep cargo on.
